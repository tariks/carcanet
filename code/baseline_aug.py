...
def read_input(path):
    x = resize(cv2.imread(path) / 255.0, input_shape)
    return np.asarray(x)


def read_gt(path):
    y = resize(cv2.imread(path, 0) / 255.0, input_shape)
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-d", "--dropout", required=False, help="dropout", type=float, default=0
    )
    ap.add_argument(
        "-a", "--activation", required=False, help="activation", default="ReLu"
    )
    ...
    model_name = "baseline_unet_aug_do_%s_activation_%s_" % (
        args["dropout"],
        args["activation"],
    )
    ...
    train_data = list(
        zip(
            sorted(glob("../input/training_input/*.jpg")),
            sorted(glob("../input/training_ground-truth/*.jpg")),
        )
    )
    val_data = list(
        zip(
            sorted(glob("../input/validation_input/*.jpg")),
            sorted(glob("../input/validation_ground-truth/*.jpg")),
        )
    )
